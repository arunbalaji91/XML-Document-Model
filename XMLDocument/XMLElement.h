...
namespace XmlProcessing
{
	...
	public:
		...
		virtual void getNumofChilds(int v) = 0;
		virtual int numofChild() = 0;

		virtual int isSelfClose() = 0;
		virtual void selfCloseTag(int tf) = 0;

		virtual std::vector<std::shared_ptr<AbstractXmlElement>> children() = 0;
		virtual std::vector<std::pair<std::string, std::string>> attributes() = 0;
	...
	public:
		...
		DocElement(const DocElement&&);
		DocElement& operator=(const DocElement&&);
		...
		virtual void getNumofChilds(int){}
		virtual int numofChild(){ return 0; }

		virtual int isSelfClose(){ return 0; }
		virtual void selfCloseTag(int tf){}

		virtual std::vector<std::shared_ptr<AbstractXmlElement>> children(){ return children_; }
		virtual std::vector<std::pair<std::string, std::string>> attributes() { std::vector<std::pair<std::string, std::string>> v; v.empty(); return v; }
	...
	public:
		...
		virtual std::vector<std::shared_ptr<AbstractXmlElement>> children(){ std::vector<std::shared_ptr<AbstractXmlElement>> v; v.empty(); return v; }
		virtual std::vector<std::pair<std::string, std::string>> attributes() { std::vector<std::pair<std::string, std::string>> v; v.empty(); return v; }
	...
	public:
		...
		TaggedElement(const TaggedElement&& newtagelement)
		{
			tag_= newtagelement.tag_;
			attribs_ = newtagelement.attribs_;
			newtagelement.tag_.empty();
			newtagelement.attribs_.empty();
		}

		TaggedElement& operator=(const TaggedElement&& newtagelement)
		{
			if (this != &newtagelement)
			{
				tag_ = newtagelement.tag_;
				attribs_ = newtagelement.attribs_;
				newtagelement.tag_.empty();
		...
		virtual int numofChild(){ return numchld; }
		virtual void getNumofChilds(int v){ numchld = v; }

		virtual int isSelfClose(){ return selfClose; }
		virtual void selfCloseTag(int tf){ selfClose = tf; }

		virtual std::vector<std::shared_ptr<AbstractXmlElement>> children(){ return children_; }
		virtual std::vector<std::pair<std::string, std::string>> attributes(){ return attribs_; }
	...
	public:
		...
		CommentElement(const CommentElement&&);
		CommentElement& operator=(const CommentElement&&);
	...
	public:
		...
		ProcInstrElement(const ProcInstrElement&&);
		ProcInstrElement& operator=(const ProcInstrElement&&);
	...
	public:
		...
		XmlDeclarElement(const std::string& type) : type_(type){}
		XmlDeclarElement& operator=(const ProcInstrElement& pe) = delete;

		XmlDeclarElement(const XmlDeclarElement&&);
		XmlDeclarElement& operator=(const XmlDeclarElement&&);
	...
	std::shared_ptr<AbstractXmlElement> makeXmlDeclarElement(const std::string& type);
